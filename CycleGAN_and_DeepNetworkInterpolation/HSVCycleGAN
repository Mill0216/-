...
class preprocessed_data(data.Dataset):
    def __init__(self, dataset_name, img_size,
                 train, augumentation, colab, make_dir=False):
        ...
        for i, AorB in enumerate(['A', 'B']):
            self.data_filepath_lists[i] += glob.glob("{}/{}/{}{}/*".format("datasets" \
                if not colab else "drive/My Drive/Colab Notebooks/HSVCycleGAN/datasets",
    ...
    def to_tensor(self, i):
        ...
        def tothsv(np_img):
            nrow = len(np_img)
            nline = len(np_img[0])
            returns = np.empty((nrow, nline, 4), dtype=np.float)
            for i in range(nrow):
                for j in range(nline):
                    ...
                    if H < 0:
                        H += 6
                    H = rad(H*60)                 # to radian
                    Hsin = (math.sin(H)+1)*127.5  # to [0,255]
                    Hcos = (math.cos(H)+1)*127.5
                    if MAX != 0:
                        S = (MAX-MIN)/MAX*255
                    else:
                        S = 0
                    V = MAX
                    returns[i][j] = np.array([Hsin, Hcos, S, V])

            returns = np.array(returns, dtype='uint8')
            return returns
        ...
        for AorB, data_filepath_list in zip(['A', 'B'], self.data_filepath_lists):
            with open(data_filepath_list[i], 'rb') as f:
                ...
                np_img = np.array(self.transform(img.convert("RGB")), dtype='uint8')
                tr_img = tothsv(np_img)
                tensor_img = (self.toTensor(tr_img)*2-1)\
                                    .view(1, 4, self.img_size[0], self.img_size[1])
...
class ResnetGenerator(nn.Module):
    def __init__(self, img_size):
        ...
        input_channel = 4
        n_blocks = 6 if img_size[0] <= 128 else 9
        ...
        model += [
            nn.ReflectionPad2d(3),
            nn.Conv2d(64, 4, kernel_size=7, padding=0),
            nn.Tanh()
        ]
...
class CycleGANmodel():
    def __init__(self, img_size, identity):
        """
            netGA : AtoB
            netGB : BtoA
        ...
        self.netGA = initialize_network(ResnetGenerator(img_size))
        self.netGB = initialize_network(ResnetGenerator(img_size))
        # Discriminator's argument, input_nc, is netG's output_nc
        self.netDA = initialize_network(Discriminator())
        self.netDB = initialize_network(Discriminator())
...
class running():
    def __init__(self, colab=False, img_size=(256, 256), identity=True):
        ...
        self.MODEL_SAVE_DIR = "drive/My Drive/Colab Notebooks/HSVCycleGAN/model's parameters"\
                                    if colab else "models"
        
    def train(self, origin_dirname=None, ran_epoch=0,
        ...
        def progress_bar(i, datasize, phase):
            ...
            print("\r[{0}] epoch{1} {2}".format("＞"*per20 + "＿"*(20-per20),
                                                epoch+1, condition), end="")
        ...
        for epoch in range(ran_epoch, max_epoch):
            ...
            train_dataset = preprocessed_data(deriv_dirname, train=True,
                                augumentation=augumentation, **self.dataset_arguments)
            train_losses = []
            ...
            test_dataset = preprocessed_data(deriv_dirname, train=False,
                                    augumentation=False, **self.dataset_arguments)
            valid_losses = []
    ...
    def make_directory(self, trial=False, origin_dirname=None,
        ...
        test_dataset = preprocessed_data(deriv_dirname, train=phase == 'train',
                            augumentation=False, make_dir=True, **self.dataset_arguments)
        ...
        def predict_and_imgsave(interpolate, test_data, alpha=0):
            ...
            def toDegree(sin, cos):
                degree_from_sin = round(math.asin(sin) / math.pi * 180)
                degree_from_cos = round(math.acos(cos) / math.pi * 180)

                if sin >= 0:
                    if cos >= 0:
                        return round((degree_from_sin + degree_from_cos) / 2)
                    else:
                        return round((180 - degree_from_sin + degree_from_cos) / 2)
                else:
                    if cos < 0:
                        return round((540 - degree_from_sin - degree_from_cos) / 2)
                    else:
                        return round((720 + degree_from_sin - degree_from_cos) / 2)
                    
            def torgb(np_img):
                nrow = len(np_img[0])
                nline = len(np_img[0, 0])
                returns = np.empty((nrow, nline, 3), dtype=np.int)
                for i in range(nrow):
                    for j in range(nline):
                        Hsin = np_img[0, i, j]
                        Hcos = np_img[1, i, j]
                        H = toDegree(Hsin, Hcos)
                        S = (np_img[2, i, j]+1)*127.5
                        V = (np_img[3, i, j]+1)*127.5
                        MAX = V
                        MIN = MAX-((S/255)*MAX)
                        if H < 60:
                            r = MAX
            ...
            fake_imgs = [
                Image.fromarray(np.array(np.reshape(torgb(
                    tensor_img.detach().cpu().clone().squeeze().numpy()),
                        (256, 256, 3)), dtype='uint8'))
                            for tensor_img in fake_tensors
            ]
            ...
            for AorB, fake_img in zip(['B', 'A'], fake_imgs):
                if interpolate:
                    ...
                    replaced_name = "datasets/" if not self.colab \
                        else "drive/My Drive/Colab Notebooks/HSVCycleGAN/datasets/"+deriv_dirname
                    ...
                    savedir_name = os.path.join(MODEL_SAVE_DIR, '{}{}(epoch{}'.format(self.dirheader,
                            AorB+phase, ran_epoch if deriv_ran_epoch == None else deriv_ran_epoch))
                    os.makedirs(savedir_name, exist_ok=True)
